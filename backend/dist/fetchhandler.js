...
exports.fetch_add_params = fetch_add_params;
exports.fetch_data_generic = fetch_data_generic;
function fetch_add_params(p_params_as_json) {
	var l_esc = encodeURIComponent;
	var l_retval = Object.keys(p_params_as_json).map(function (k) {
		return l_esc(k) + '=' + l_esc(p_params_as_json[k]);
	}).join('&');
	return l_retval;
}

function fetch_data_generic(p_function_to_execute_with_result_json, p_uri, p_method, p_params_as_json, p_cross_or_same_origin, p_origin) {
	...
	fetch(l_uri, l_Init).then(function (response) {
