...
exports.compare_by_intid = compare_by_intid;
exports.sort_arrayofhash = sort_arrayofhash;
exports.sort_arrayofhash_byfield = sort_arrayofhash_byfield;
exports.dynamicSort = dynamicSort;
exports.dynamicSortMultiple = dynamicSortMultiple;
exports.filter_by_where = filter_by_where;
exports.nvl = nvl;
exports.fibonacci = fibonacci;
exports.shuffle = shuffle;
exports.checkVisible = checkVisible;
exports.mergeTwoArraysOfHash = mergeTwoArraysOfHash;
exports.removeFromArrayOfHash = removeFromArrayOfHash;
exports.getIndex = getIndex;
exports.getIndex_2 = getIndex_2;
exports.getUTCTimeAsString = getUTCTimeAsString;
exports.str_to_date = str_to_date;
exports.date_to_str = date_to_str;
function compare_by_intid(a, b) {
	if (parseInt(a.id) < parseInt(b.id)) return -1;
	if (parseInt(a.id) > parseInt(b.id)) return 1;
	return 0;
}

function sort_arrayofhash(p_input) {
...
function sort_arrayofhash_byfield(p_input_as_array, p_field_name_as_string, p_get_compare_value_as_function) {
	return p_input_as_array.sort(function (p_left, p_right) {
		...
		if (l_left_value < l_right_value) return -1;
		if (l_left_value > l_right_value) return 1;
		return 0;
	});
}
...
function filter_by_where(p_array_of_hashes, p_conditions_as_string) {
...
function mergeTwoArraysOfHash(p_recessive_arr, p_dominant_arr, p_columnname) {
	var l_temp_arr = p_recessive_arr.filter(function (l_cur_hash) {
		var l_conditions = "(p_cur_item." + p_columnname + " == '" + l_cur_hash[p_columnname] + "')";
...
function removeFromArrayOfHash(p_array_of_hash, p_key, p_value_to_remove) {
	return p_array_of_hash.filter(function (l_cur_row) {
 ...
 */
function getIndex(p_arr, p_hash_key, p_value) {
...
function getIndex_2(p_arr, p_value) {
	for (var i = 0; i < p_arr.length; i++) {
		if (p_arr[i] === p_value) {
			return i;
		}
...
function str_to_date(p_instr) {
	// expected input format: 1976-12-23
	var p_instr_arr = p_instr.split("-");
	return new Date(parseInt(p_instr_arr[0]), parseInt(p_instr_arr[1]) - 1, // javascript starts indexing months from 0. wierd. :-)
...
function date_to_str(p_indate, p_format) {
	var z = {
